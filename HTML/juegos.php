						...
						<?php
							if(isset($_GET['busqueda'])){
								...
								if($genero=="Ninguno"){
									...
									*/
									 $sql = "select id_juego, imagen, nombre, year, id_juego_a_puntuacion(id_juego) puntuacion from juegos where 
									 		upper(nombre) like upper('%".$cad."%') and 
									 		year>=to_date('".$inicio_year."', 'yyyy') and 
									 		year<=to_date('".($fin_year+1)."', 'yyyy')";
								...
								$sql2="select id_juego, imagen, nombre, year, puntuacion from
											(select id_juego, imagen, nombre, year, puntuacion, rownum rn from
											(".$sql.") 
								...
								echo '</ul>
									<ul id="paginacion">';
								if($pagina>1){
									echo '<li><a href="juegos.php?busqueda='.$cad.'&inicio_year='.$inicio_year.'&fin_year='.$fin_year.'&genero='.$genero.'&pagina=1&juegos='.$pelisPorPaginas.'&orden='.$_GET['orden'].'&torden='.$_GET["torden"].'"><<</a></li>
										<li><a href="juegos.php?busqueda='.$cad.'&inicio_year='.$inicio_year.'&fin_year='.$fin_year.'&genero='.$genero.'&pagina='.($pagina-1).'&juegos='.$pelisPorPaginas.'&orden='.$_GET['orden'].'&torden='.$_GET["torden"].'"><</a></li>';
								}
								$i = 0;
								while ($cont>$i) {
									$i=$i+1;
									echo '<li><a href="juegos.php?busqueda='.$cad.'&inicio_year='.$inicio_year.'&fin_year='.$fin_year.'&genero='.$genero.'&pagina='.$i.'&juegos='.$pelisPorPaginas.'&orden='.$_GET['orden'].'&torden='.$_GET["torden"].'">'.$i.'</a></li>';
										
								}
								if($pagina<$cont){
									echo '<li><a href="juegos.php?busqueda='.$cad.'&inicio_year='.$inicio_year.'&fin_year='.$fin_year.'&genero='.$genero.'&pagina='.($pagina+1).'&juegos='.$pelisPorPaginas.'&orden='.$_GET['orden'].'&torden='.$_GET["torden"].'">></a></li>
										<li><a href="juegos.php?busqueda='.$cad.'&inicio_year='.$inicio_year.'&fin_year='.$fin_year.'&genero='.$genero.'&pagina='.ceil($cont).'&juegos='.$pelisPorPaginas.'&orden='.$_GET['orden'].'&torden='.$_GET["torden"].'">>></a></li>';
								}
								CerrarConexionBD($con);
							}else{
								echo '</table>';
