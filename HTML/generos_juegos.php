		...
		<section id="seccion">
			...
			<?php
				if($_SESSION['dni'] == '00000000A' && isset($_POST['modificar'])){
					...
					$sql = "update generos_juegos set genero = '$nuevo' where genero = '$anterior'";
					$res = $con->exec($sql);
					if($res){
			...
			<?php
				if($_SESSION['dni'] == '00000000A' && isset($_POST['borrar'])){
					...
					$sql = "delete from generos_juegos where genero = '$anterior'";
					$res = $con->exec($sql);
					if($res){
			...
			<?php
				...
				if($_SESSION['dni']=='00000000A'){
					...
					echo'<tr>
						<td><form method="POST" action = "generos_juegos.php">
