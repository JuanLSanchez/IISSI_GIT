	...
	<header id="cabecera">
		...
		if(isset($_SESSION['dni']) && isset($_GET['dni'])){
			...
			if($res || $dni == $usuario){
				...
				$sql = "select id_pelicula, id_pelicula_a_nombre(id_pelicula) from peliculas_vistas where dni='$dni'";
				foreach($con->query($sql) as $fila){			
					echo'<li>
				...
				$sql = "select id_juego, id_juego_a_nombre(id_juego) from juegos_vistos where dni='$dni'";
				foreach($con->query($sql) as $fila){			
					echo'<li>
